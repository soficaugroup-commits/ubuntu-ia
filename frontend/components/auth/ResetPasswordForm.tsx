...
export function ResetPasswordForm({ token }: { token: string }) {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (password.length < 8) {
      setPasswordError(copy.passwordReset.tooShort);
      return;
    }
    if (password !== confirmation) {
      setConfirmError(copy.passwordReset.mismatch);
      return;
    }

    setPending(true);
    const response = await fetch(`/api/password-reset/${token}/confirm`, {
      method: "POST",
  ...
  if (state !== "ready" || !info) {
    const title =
      state === "expired"
        ...
        : state === "used"
          ? copy.passwordReset.usedTitle
          : copy.passwordReset.invalidTitle;
    const body =
      state === "expired"
        ? copy.passwordReset.expiredBody
        : state === "used"
          ? copy.passwordReset.usedBody
          : copy.passwordReset.invalidBody;
    return (
      <main className="mx-auto flex min-h-full max-w-lg flex-col justify-center px-3 py-10 sm:px-4 sm:py-16">
        <EmptyState
          ...
          body={body}
          action={
            <Tooltip label={copy.passwordReset.tipBackAction}>
              <Link
                href={state === "expired" || state === "used" ? "/mot-de-passe-oublie" : "/connexion"}
              ...
              >
                {state === "expired" || state === "used"
                  ? copy.passwordReset.requestAgain
      ...
      <Surface elevation="raised" radius="card" className="p-5 sm:p-8">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <h1 className="text-xl font-semibold text-content sm:text-2xl">
              {copy.passwordReset.confirmTitle}
            </h1>
          </div>
        </div>
        <p className="mb-6 text-content-muted">
          {interpolate(copy.passwordReset.confirmLead, { name: displayName })}
        </p>
        <form className="flex flex-col gap-4" onSubmit={(event) => void onSubmit(event)}>
          <TextField
            ...
            label={copy.passwordReset.passwordLabel}
            hint={copy.passwordReset.passwordHint}
            value={password}
            error={passwordError}
          ...
          <TextField
            ...
            type="password"
            autoComplete="new-password"
            label={copy.passwordReset.confirmLabel}
            value={confirmation}
            error={confirmError}
          ...
          <Button type="submit" pending={pending} tooltip={copy.passwordReset.tipConfirm}>
            {pending ? copy.passwordReset.confirmSubmitting : copy.passwordReset.confirmSubmit}
          </Button>
        </form>
      </Surface>
    </main>
